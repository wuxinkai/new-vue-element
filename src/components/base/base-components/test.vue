<template>
  <div>
    <el-tag v-for="item in currValue" :key="item.key" closable :disable-transitions="false" size="small" @close="deleteTag(item.key)">
      <b>{{ item.key + ': ' }}</b>{{ item.value }}
    </el-tag>
    <el-select v-if="!selected" v-model="selected" filterable>
      <el-option v-for="item in currOptions" v-show="!item.selected" :key="item.value" :value="item.value" :label="item.label" />
    </el-select>
    <el-input v-else v-model="inputValue" style="width: 300px" @keyup.enter.native="inputResultConfirm" @blur="inputResultConfirm">
