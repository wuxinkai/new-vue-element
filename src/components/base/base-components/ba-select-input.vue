<template>
  <div>
    <el-tag v-for="tag in currValue" :key="tag.key" closable @close="deleteTag(tag.key)">
      {{ `${tag.key}: ${tag.value}`}}
    </el-tag>
    <el-select v-if="!selectValue" v-model="selectValue" filterable>
      <el-option v-for="item in currOptions" v-show="!item.selected" :key="item.key" :label="item.label" :value="item.key" />
    </el-select>
    <el-input v-else v-model="inputValue" @blur="onConfirmResult" @keyup.enter.native="onConfirmResult" class="input" clearable>
